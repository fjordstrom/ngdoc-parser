...
module.exports = function extractTypeTransform(value) {
  ...
  if (start !== -1) {
    ...
    var typeExp = res.typeExpression;
    var index = typeExp.indexOf('=');

    if(index > -1) {
        res.defaultValue = typeExp.substr(index+1);
        typeExp = typeExp.substr(0,index);
    }

    res.type = catharsis.parse(typeExp, {jsdoc: true});
